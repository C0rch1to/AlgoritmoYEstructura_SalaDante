...
class ListaEnlazada:
    ...
    def eliminar(self, clave_nombre: str):
        """
        Elimina el nodo cuyo superhéroe coincide con el nombre indicado.
        ...
        if anterior is None:
            # El nodo a eliminar es el primero (inicio)
            self.inicio = actual.sig
        else:
            # Se puentea el nodo actual
    ...
    def buscar(self, clave_nombre: str):
        """Busca un superhéroe por nombre y retorna el objeto Superheroe o None."""
        actual = self.inicio
